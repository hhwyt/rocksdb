...
TEST_P(ExternalSSTFileTest, WriteDuringIngest) {
  ...
  // Setup options with unordered write enabled
  Options options = CurrentOptions();
  DestroyAndReopen(options);

  SyncPoint::GetInstance()->EnableProcessing();
  ASSERT_OK(GenerateAndAddExternalFile(options, {{"foo", "v1"}}, -1,
                                       true /* allow_global_seqno */, false, true, false, false, true /* allow_write */));
